 ...
 */
public class DropMessagePerNodeReport extends Report implements MessageListener {
	private Map<String, Double> creationTimes;
	private List<Double> latencies;
	private List<Integer> hopCounts;
	private List<Double> msgBufferTime;
	private List<Double> rtt; // round trip times
        private Map<DTNHost, Integer> dropnode; //DEKLARASI
	 ...
	 * Constructor.
	 */
	public DropMessagePerNodeReport() {
		init();
	}
	...
	protected void init() {
		...
		this.dropnode = new HashMap<DTNHost, Integer>();
	...
	public void messageDeleted(Message m, DTNHost where, boolean dropped) {
		...
		if (dropped) {
			if(dropnode.containsKey(where)) {
                            Integer count = dropnode.get(where);
                            dropnode.put(where, count+1);
                        } else {
                            dropnode.put(where, 1);
                        }
                        this.nrofDropped++;
                        //cara ngecek map punya key tertentu
                        //jika punya, ambil valuenya, tambahkan 1, simpan ke map
                        // kl belum, bikin value = 1
	...
	public void done() {
		...
		String statsText = "";
                for (Map.Entry<DTNHost, Integer> entry : this.dropnode.entrySet()) {
                    DTNHost k = entry.getKey();
...
//Drop per node per satuan waktu.
// K (DTN Host), V (List)
//node dropped time
// 0   5        10 min